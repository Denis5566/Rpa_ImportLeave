      ...
      Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "LOG");
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                filePath = filePath.Replace("{Date}", DateTime.Today.ToString("yyyy_MM_dd"));
                if (string.IsNullOrEmpty(filePath))
                {
                    WriteLog("ERROR：未設定ExcelPath");
                    return;
                }

                if (!File.Exists(filePath))
                {
                    WriteLog("ERROR：檔案不存在：" + filePath);
                    return;
                }
                int successCount = 0;
                int failCount = 0;
                    ...
                    new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    WriteLog("開始刪除 D_SOURCE='A' 舊資料");
                    DeleteOldData();
                    WriteLog("刪除完成");

                    const int COL_STATUS = 16;
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            WriteLog(
                                $"失敗 Row={row} 工號:{ws.Cells[row, 2].Text}",true);
...
WHERE D_SOURCE = 'A'";
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    ...
                    WriteLog($"已刪除 {count} 筆 D_SOURCE='A' 資料");
                }
            }
        }
        #endregion
